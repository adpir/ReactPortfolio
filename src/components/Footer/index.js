...
const navigation = {
    ...
    social: [
        {
            name: 'Linkedin',
            href: "https://www.linkedin.com/in/alicepiar/",
            icon: (props) => (
...
export default function Footer() {
    return (
        <footer className="bg-gradient-to-r from-gray-800 via-blue-600 to-cyan-400 ">
            <div className="max-w-7xl mx-auto py-12 px-4 overflow-hidden sm:px-6 lg:px-8">
                <nav className="-mx-5 -my-2 flex flex-wrap justify-center" aria-label="Footer">
                    {navigation.main.map((item) => (
                        <div key={item.name} className="px-5 py-2">
                            <a href={item.href} className="text-base text-yellow-500 hover:text-white">
                                {item.name}
                            </a>
                        </div>
                    ))}
